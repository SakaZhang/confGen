...
type KodoBillCsv struct {
	BaseStruct
}

func (u KodoBillCsv) ToCSV() ([]string, error) {
	var csv []string
...
func (u *KodoBillCsv) UnmarshalYAML(value *yaml.Node) error {
	tmp := getLastConf("kodobill")
	lastConf, ok := tmp.(KodoBillCsv)
	if !ok {
